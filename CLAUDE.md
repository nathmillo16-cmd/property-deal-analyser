...
- Planning constraints panel — Article 4 wording decision (next session): see "Planning constraints panel" above. Soften the Article 4 row's copy given confirmed incomplete national coverage on planning.data.gov.uk (Broxtowe has zero records despite a real designation existing) — proposed: "None recorded nationally — verify with local authority for HMO deals" instead of a confident "None found". Not yet decided/implemented.
