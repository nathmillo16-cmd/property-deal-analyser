...
- Crime rate: free, via Police UK's open data by location. Police UK's API takes lat/lng directly, and where MSOA-level crime data is needed instead, that resolves live from the same postcodes.io subject-postcode lookup the comps engine already makes (see Comps engine above) — no stored geography column required, same live-resolution approach throughout.
