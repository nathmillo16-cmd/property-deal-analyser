...
- Selective licensing flag: NOT on planning.data.gov.uk — confirmed while building the Planning constraints panel (administered per-council, no central dataset; corrects the earlier research note above, which had assumed partial coverage there). Would need a different, per-council data source; still parked as a separate future feature, not an extension of the Planning constraints panel.
